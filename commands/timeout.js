...
const Database = require('../utils/infractions')
const ms = require('ms');
...
module.exports = {
    ...
    .addStringOption(options => options
        .setName('duration')
        .setDescription('Provide a duration to timeout (1m,1h,1d)')
        .setRequired(true)
    )
    .addStringOption(options => options
        .setName('reason')
    ...
    async execute(interaction) {
        ...
        const errorsEmbed = new EmbedBuilder().setAuthor({name: "Could not timeout member due to"})
        .setColor('Red');
